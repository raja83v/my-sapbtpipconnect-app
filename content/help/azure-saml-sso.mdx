---
title: Configure CPI Connect 
updatedAt: 2025-05-09
summary: Connect Azure Active Directory to CPI Connect so your team signs in with existing corporate credentials.
author: codehagen
categories:
  - analysis
slug: azure-saml-sso
...
<Note variant="success">
  SAML single sign-on is available on CPI Connect Scale and Enterprise plans.
</Note>

CPI Connect Scale customers can enforce single sign-on through Azure Active Directory using the SAML 2.0 protocol. The steps below walk through the Azure configuration and the fields CPI Connect expects.
...
In the next screen, click on **Create your own application**. Give your application a **Name** (e.g. "CPI Connect") and click **Create**.
...
Scroll down to the 3rd section on the page, **SAML Certificates**. Copy the **App Federation Metadata Url** value—CPI Connect uses it to keep your certificate in sync.
...
## Step 5: Configure SAML SSO on CPI Connect

In your CPI Connect workspace, open **Settings → Security**.

<Image
  alt="SAML SSO section on the CPI Connect dashboard"
...
Under **SAML Single Sign-On**, click **Configure**. This opens the CPI Connect SSO modal:
...
Your assigned users should now receive an invitation email to join your CPI Connect workspace.
...
<Note variant="warning">
  ...
  invitation email and be able to join your CPI Connect workspace.
</Note>

They will also be able to sign in to CPI Connect using Azure AD SSO.
