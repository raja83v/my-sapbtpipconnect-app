/**
 * Encryption utilities for Convex backend
 *
 * This module provides encryption/decryption functions that work in the Convex runtime.
 * Uses AES-256-GCM algorithm with Web Crypto API (no Node.js dependencies).
 */
...
function getEncryptionKey(): string {
    ...
    if (!key) {
        throw new Error(
            ...
            'Set it in your Convex dashboard or use: npx convex env set ENCRYPTION_KEY <key>'
        );
    }
    return key;
}
...
/**
 * Decrypt a client secret for use in OAuth flows
 */
export async function decryptClientSecret(encryptedSecret: string): Promise<string> {
